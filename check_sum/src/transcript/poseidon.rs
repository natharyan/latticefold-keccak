...
use lattirust_arithmetic::{
    challenge_set::latticefold_challenge_set::{LatticefoldChallengeSet, OverField}, ring::UnsignedRepresentative
};
...
fn ring_to_field<F: PrimeField, R: OverField<F>>(x: &R) -> Vec<F> {
    x.coeffs()
        ...
        .map(|coeff| {
            F::from(
                <<R>::BaseRing as Into<UnsignedRepresentative>>::into(coeff)
                    .0,
            )
        })
        .collect()
