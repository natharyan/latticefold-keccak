    ...
    F: Absorb,
{
    ...
    pub fn verify(&self) -> Result<SumCheckSubClaim<F, R>, SumcheckError<R>> {
        let mut check_sum = self.transcript.claimed_sum;
        let poly = &self.transcript.polynomial;
        for round in self.transcript.rounds.iter() {
            ...
            } else if round.unipoly.aux_info.max_degree > poly.aux_info.max_degree {
                return Err(SumcheckError::MaxDegreeExcided);
            }
            
            check_sum = round.unipoly.evaluate(&[round.challenge])?;
            
        }
        let challenge_vector: Vec<R> = self.transcript.rounds.iter().map(|x| x.challenge).collect();
...
mod test {
    ...
    use crate::utils::sumcheck::prover::SumCheckProver;
    ...
    fn test_sumcheck_protocol() {
        ...
        let subclaim = verifier.verify().unwrap();
        assert!(
