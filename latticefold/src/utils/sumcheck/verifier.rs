...
pub struct SumCheckSubClaim<F: Field, R: OverField<F>> {
...
pub struct SumCheckVerifier<F: Field, R: OverField<F>, CS: LatticefoldChallengeSet<F, R>>
where
    F: Absorb,
{
...
impl<F: Field, R: OverField<F>, CS: LatticefoldChallengeSet<F, R>> SumCheckVerifier<F, R, CS>
where
    F: Absorb,
{
